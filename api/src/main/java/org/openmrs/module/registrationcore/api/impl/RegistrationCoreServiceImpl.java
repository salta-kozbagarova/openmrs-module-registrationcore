...
import org.openmrs.module.registrationcore.api.search.PatientIINSearch;
...
public class RegistrationCoreServiceImpl extends BaseOpenmrsService implements RegistrationCoreService, GlobalPropertyListener, ApplicationContextAware {
	...
	private PatientIINSearch getPatientIINSearch() {
		String gp = adminService.getGlobalProperty(RegistrationCoreConstants.GP_PATIENT_IIN_SEARCH,
		    "registrationcore.BasicPatientIINSearch");
		
		Object bean = applicationContext.getBean(gp);
		if (bean instanceof PatientIINSearch) {
			return (PatientIINSearch) bean;
		} else {
			throw new IllegalArgumentException(RegistrationCoreConstants.GP_PATIENT_IIN_SEARCH + " must point to "
			        + "a bean implementing PatientIINSearch");
		}
	}
	...
	public Boolean isIINUnique(String iin) {
		return getPatientIINSearch().isIINUnique(iin);
	}
}